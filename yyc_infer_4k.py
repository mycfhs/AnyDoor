...
# 读取YAML文件
with open("categories_final.yml", "r") as yml_file:
...
seed = 42  #
# special_tag = ""
train_data_dir = "/home/dhw/yyc_workspace/project/Fooocus_diffusers/train_data"
...
if __name__ == '__main__': 
    ...
    bg_dir = "/home/dhw/yyc_workspace/project/Fooocus_diffusers/bg_images"
    all_bg = [png[:-6] for png in os.listdir(bg_dir) if png.endswith("_1.png")]
    category_list = [i for i in os.listdir(train_data_dir) if i in category_dict.keys()]
    category_num = 30
    ...
    for index, category in enumerate(tqdm(category_list)):
        ...
        folder_path = f"./output/anydoor_bg4k_dinob/{category}"
        ...
        for img_name in all_bg[index :: category_num]:
            try:
                ...
                ref_image_url = f"/home/dhw/yyc_workspace/project/Fooocus_diffusers/train_data/{category}/image/00.jpg"
                ref_image = cv2.imread(ref_image_url)
                # ref_image = cv2.cvtColor(ref_image, cv2.COLOR_BGR2RGB)

                ref_mask_url = f"/home/dhw/yyc_workspace/project/Fooocus_diffusers/train_data/{category}/mask/00.png"
                img_url = f"/home/dhw/yyc_workspace/project/Fooocus_diffusers/bg_images/{img_name}_1.png"
                mask_url = f"/home/dhw/yyc_workspace/project/Fooocus_diffusers/bg_images/{img_name}_2.png"
                gt_image = cv2.imread(img_url)
                # gt_image = cv2.cvtColor(gt_image, cv2.COLOR_BGR2RGB)

                ref_mask = (cv2.imread(ref_mask_url) > 128).astype(np.uint8)[:,:,0]
                tar_mask = (cv2.imread(mask_url) > 128).astype(np.uint8)[:, :, 0]
                
                seed_everything(seed)
                gen_image = inference_single_image(
                    ref_image, ref_mask, gt_image.copy(), tar_mask
                )
                gen_path = f"./{folder_path}/{img_name}+None+{seed}.jpg"

                # vis_image = cv2.hconcat([ref_image, gt_image, gen_image])
                cv2.imwrite(gen_path, gen_image)
            except Exception as e:
                error_file.append(f"{category}_{img_name}")
                continue
